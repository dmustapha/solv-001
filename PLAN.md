...
**Architecture Doc:** /Users/MAC/agora-agents/solv-001/ARCHITECTURE.md (THE source of truth for all code)
...
1. Create the Next.js project:
   ```bash
   cd /Users/MAC/agora-agents/solv-001
...
2. Submit a support ticket with subject: "USYC Allowlisting for Arc Testnet — solv-001 Hackathon"
...
1. Follow Task 1.5 steps — USYC APY read-only path works without allowlisting
...
3. Use APY-only display: `getUSYCPosition` reads `getApy()` from the Teller contract (does not require allowlisting)
...
2. Verify the agent card is served correctly:
   ...
   ```
   Expected: JSON with `name: "solv-001"`, `capabilities` array, `pricing` object
